...
export default function GRVDetailPage({ params }: GRVDetailPageProps) {
  ...
  const handlePost = async () => {
    ...
    try {
      const response = await fetch(`/api/grvs/${grv._id}/post`, {
        method: "POST",
      });
      
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.error || "Failed to post GRV");
      }
      ...
      toast({ 
        title: "Error", 
        description: err instanceof Error ? err.message : "Failed to post GRV",
        variant: "destructive"
      });
    }
  };
